...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (nutri) {
        ...
        await convex.mutation(api.Slots.ensureSlots, {
        ...
        const upcoming = allConsultations.filter(
          (c) =>
            c.status === "confirmed" ||
            c.status === "pending" ||
            c.status === "upcoming",
        );
        const completedToday = allConsultations.filter(
        ...
        setStats({
          ...
          totalClients: [...new Set(allConsultations.map((c) => c.user?._id))]
            .length,
          pendingPlans: allConsultations.filter(
            (c) => c.status === "completed" && !c.expertDietPlan,
          ).length,
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: (
        <Ionicons name="document-text" size={22} color={theme.colors.warning} />
      ),
    ...
    >
      ...
      <View style={styles.quickActions}>
        <TouchableOpacity
          ...
          onPress={() =>
            router.push(`/consultancy/consultation/${item._id}/start`)
          }
        >
